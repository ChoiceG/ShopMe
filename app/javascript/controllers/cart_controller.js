...
//   checkout() {
//     const isLoggedIn = document.querySelector("meta[name='current-user']").content === "true";

//     if (!isLoggedIn) {
//       alert("Please sign in to complete your checkout.");
//       window.location.href = "/users/sign_in"; // Devise default login path
//       return;
//     }
...
export default class CartController extends Controller {
  ...
  displayCart() {
    ...
    if (cart.length === 0) {
      ...
      emptyCartMessage.classList.add('text-center', 'text-lg', 'text-gray-500');
      emptyCartMessage.innerText = "Your cart is empty. Add items to your cart before proceeding to checkout.";
      totalEl.before(emptyCartMessage);
    }
  ...
  // Checkout function
  checkout() {
    // Get the value of the meta tag to check if the user is logged in
    const isLoggedIn = document.querySelector("meta[name='current-user']").content === "true";
    
    // If the user is not logged in, redirect to sign in page
    if (!isLoggedIn) {
      const isSignedUp = document.querySelector("meta[name='signed-up']").content === "true";
  
      if (!isSignedUp) {
        // If user is not signed up, redirect to sign-up page
        alert("Please sign up to complete your checkout.");
        window.location.href = "/users/sign_up"; // Redirect to sign-up page
        return;
      } else {
        // If the user is not signed in but signed up, redirect to sign-in page
        alert("Please sign in to complete your checkout.");
    ...
    // If the user is logged in, continue with the checkout process
    const cart = JSON.parse(localStorage.getItem("cart"));
    const payload = {
      authenticity: "",
      cart: cart
    ...
    // Make the fetch request to initiate the checkout
    fetch("/checkout", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
